...
def main(args):
    ...
    if args.evaluate:
        ...
        for i, action in enumerate(action_filter):
            poses_valid, poses_valid_2d, actions_valid, intrinsics_valid = fetch(subjects_test, dataset, keypoints, [action], stride)
            valid_loader = DataLoader(PoseGenerator(poses_valid, poses_valid_2d, actions_valid, intrinsics_valid),
    ...
    poses_train, poses_train_2d, actions_train, intrinsics_train = fetch(subjects_train, dataset, keypoints, action_filter, stride)
    train_loader = DataLoader(PoseGenerator(poses_train, poses_train_2d, actions_train, intrinsics_train), batch_size=args.batch_size,
                              shuffle=True, num_workers=args.num_workers, pin_memory=True)

    poses_valid, poses_valid_2d, actions_valid, intrinsics_valid = fetch(subjects_test, dataset, keypoints, action_filter, stride)
    valid_loader = DataLoader(PoseGenerator(poses_valid, poses_valid_2d, actions_valid, intrinsics_valid), batch_size=args.batch_size,
